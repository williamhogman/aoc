...
For example, suppose the starting numbers are 0,3,6:
    ...
    * Turn 5: Next, again consider the last number spoken, 0. Since it had
      ...
      4) and the turn number of the time it was most recently spoken before
...
fn exercise(input: &[u16], upto: u16) -> u16 {
    let mut old: HashMap<u16, (u16, u16)> = HashMap::with_capacity(upto.into());
    for (i, x) in input.iter().enumerate() {
	old.insert(*x, (i as u16, i as u16));
    }
    let mut turn: u16 = (input.len() - 1) as u16;
    let mut prev: u16 = *input.last().unwrap();

    for turn in 0..upto {
	let (next_last_mention, last_mention) = old[&prev];
	if next_last_mention == last_mention {
	    prev = 0
	} else {
	    prev = next_last_mention - last_mention
	}

	if let Some(x) = old.get_mut(&prev) {
	    *x = (x.1, turn);
	} else {
	    old.insert(prev, (turn, turn));
	}
    }
    prev
}
