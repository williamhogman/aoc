...
def find_cands(target, things, used):
    for thing in things:
...
def find_chain(adapter_chain, things, used):
    if len(used) == len(things):
        return adapter_chain
    most_recent = adapter_chain[-1]
    for cand in find_cands(most_recent, things, used):
        new_adt = adapter_chain + [cand]
        new_used = used | {cand}
        res = find_chain(new_adt, things, new_used)
        if res is not None:
            return res
    return None
...
def run_p1(things):
    ...
    res = None
    # incres = nx.shortest_path(g, 0, biggest)
    for p in nx.all_simple_paths(g, 0, biggest):
        if len(p) == len(things):
            res = p
...
@e.part1()
def part1(xs):
    things = xs.map(lambda x: x[0]).t
    things.append(0)
    things.sort()
    things.append(max(things)+3)
    count = collections.defaultdict(int)
    for i in range(1,len(things)): count[things[i]-things[i-1]]+=1
    print(count[1]*count[3])


    arrange = [1]+[0]*things[-1]
    for i in things[1:]:
