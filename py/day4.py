...
rules about what values are valid for automatic validation:
    ...
    * hgt (Height) - a number followed by either cm or in:
          o If cm, the number must be at least 150 and at most 193.
          o If in, the number must be at least 59 and at most 76.
...
At this point, you should return_to_your_Advent_calendar and try another
puzzle.
If you still want to see it, you can get_your_puzzle_input.
You can also [Shareon Twitter Mastodon] this puzzle.
"""
import re
from fastcore.all import L, Self, chunked, AttrDict
from util import *


e = Exercise(4, parser={"line_sep": "\n\n", "ints": False})
...
def transform(xs):
    return AttrDict(chunked(xs, 2))

@e.part1(map=True, reduce="count")
def part1(d):
    return all(k in d for k in fields)
...
def valid_height(hgt):
    ...
    i = force_int(hgt)
    if hgt.endswith("cm"):
        return between(i, 150, 193)
    elif hgt.endswith("in"):
        return between(i, 59, 76)
    else:
        return False
...
def part2(d):
    ...
    return all([
        between(d.byr, 1920, 2002),
        between(d.iyr, 2010, 2020),
        between(d.eyr, 2020, 2030),
        valid_height(d.hgt),
        re.match(r"^#[a-fA-F0-9]{6}$", d.hcl),
