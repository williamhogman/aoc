...
def parse_mask(mask):
    ...
    mask_xs = int(mask.replace("1", "0").replace("X", "1"), 2)

    #mask = [(36 - i, int(m)) for (i, m) in enumerate(mask) if m != "X"]

    #mask_trues = lower36 & (make_mask([i for (i, m) in mask if m == 1]))
    #mask_falses = ~(lower36 & (make_mask([i for (i, m) in mask if m == 1])))
